...
def log_graph(log, wheelRadius, DBM):
    ...
    if 'robot' in formatted_log:
        ...
        deltaTime = [t - formatted_log['time'][i-1] for i, t in enumerate(formatted_log['time'][1:])]
        ...
        if 'Pose' in robot:
            ...
            omega = [0]
            for i, theata in enumerate(robot['Pose'][2][1:]):
                if deltaTime[i] == 0: omega.append(omega[-1])
                else:
                    if theata - robot['Pose'][2][i] > pi: theata -= 2*pi
                    elif theata - robot['Pose'][2][i] < -pi: theata += 2*pi
                    omega.append((theata - robot['Pose'][2][i]) / deltaTime[i])
            omega_plot.plot(formatted_log['time'], [degrees(i) for i in omega])

        if 'Phi' in robot:
            Vl, Vr = [0], [0]
            for i, lPhi in enumerate(robot['Phi'][0][1:]):
                Vl.append(lPhi/360 * wheelCircumference / deltaTime[i])
            for i, rPhi in enumerate(robot['Phi'][1][1:]):
                Vr.append(rPhi/360 * wheelCircumference / deltaTime[i])

            Vl_plot.plot(formatted_log['time'], Vl)
            Vr_plot.plot(formatted_log['time'], Vr)

            velocity_plot.plot(formatted_log['time'], [sum(V)/2 for V in zip(Vl, Vr)])
            ...
            leftAcc_plot.plot(formatted_log['time'], AccL)
            rightAcc_plot.plot(formatted_log['time'], AccR)
...
def main():
    ...
    with open('Data/Logs/runtime.log', 'r') as f:
        log_graph(eval(f.read()), wheelRadius, DBM)
